...
pub(crate) fn infer_gpu_preset_id(config: &serde_json::Map<String, Value>) -> Option<String> {
...
async fn profile_backup_export(
    ...
    tokio::task::spawn_blocking(move || profile_backup::export(profile_ids))
        .await
        .map_err(|error| format!("backup worker failed: {error}"))?
}

#[tauri::command]
async fn profile_backup_import(
    paths: Vec<String>,
) -> Result<profile_backup::ProfileBackupSummary, String> {
